...
export default function Friends(){
    ...
    const handelSearch = async() =>{
        ...
        try {
            const res = await axios.post(
                "http://localhost:3000/friends/search",
                {
                  user_id: authData.id,
    ...
    const handelAdd = async(uid) =>{
        ...
        try {
            const res = await axios.post(
                "http://localhost:3000/friends/addfriend",
                {
                  user_id: authData.id,
                  to_user_id: uid
                }
    ...
    const handelRemove=async(uid1,uid2)=>{
        ...
        try {
            const res = await axios.post(
                "http://localhost:3000/friends/removeReq",
                {
                  user_id: uid1,
                  to_user_id: uid2
                }
    ...
    const handelAccept = async(uid1,uid2)=>{
        ...
        try {
            const res = await axios.post(
                "http://localhost:3000/friends/acceptReq",
                {
                  user_id: uid1,
                  to_user_id: uid2
                }
        ...
        <div className="px-4 py-6 w-full flex ml-32">
          <div className="grow flex-col justify-center items-center">
            ...
            <div className="flex-col text-center space-y-5">
              {friends.length !== 0
                ? friends.map((e, i) => (
                    <div className="font-medium text-xl" key={e.id}>
                      <Link to={`/writers/${e.id}`}>{e.username}</Link>
                    </div>
                  ))
          ...
          <div className="grow flex-col ">
            <div className="">
              ...
              <div className="flex-col">
                {myRequests.length !== 0
                  ? myRequests.map((e, i) => (
                      <div className="flex space-x-3 m-3" key={e.id}>
                        <Link to={`/writers/${e.id}`}>{e.username}</Link>
                        <Button
                          onClick={(er) => {
                            er.preventDefault();
                            handelRemove(authData.id, e.id);
                          }}
                        >
                          Remove
                        </Button>
