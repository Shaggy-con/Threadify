...
export default function Tags(){
    ...
    const createTag = async() => {
        setLoading(true);
        const res = await axios.post('http://localhost:3000/addtag', {tag_name:search})
        const data = res.data
    ...
    useEffect(()=>{
        ...
        const getBloggs = async()=>{
            const res = await axios.post('http://localhost:3000/alltags');
            const data = res.data
    ...
    return(<>
        <button onClick={() => { setAddingTag(Array.from(addingtags)); }} className="bg-slate-900 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mt-4">Apply Tags</button>
