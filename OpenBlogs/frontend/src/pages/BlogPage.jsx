...
export async function getVoteStats(blog_id,authData) {
  try {
    const res = await axios.get("http://localhost:3000/votes/checkvote", {
      params: {
        user_id: authData.id,
        blog_id: blog_id,
      },
...
export async function upvoteBlog(blog_id, authData , isDownvoted) {
  try {
    if(isDownvoted) {
      // delete downvote
      await axios.post("http://localhost:3000/votes/undownvote", {
        user_id: authData.id,
        blog_id: blog_id,
      });
    } else {
      // upvote
      await axios.post("http://localhost:3000/votes/upvote", {
        user_id: authData.id,
        blog_id: blog_id,
      });
    }
...
export async function downvoteBlog(blog_id, authData , isUpvoted) {
  try {
    if(isUpvoted) {
      // delete upvote
      await axios.post("http://localhost:3000/votes/unupvote", {
        user_id: authData.id,
        blog_id: blog_id,
      });
    } else {
      // downvote
      await axios.post("http://localhost:3000/votes/downvote", {
        user_id: authData.id,
        blog_id: blog_id,
      });
    }
